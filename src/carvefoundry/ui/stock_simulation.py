...
class StockSimulationMixin:
    ...
    def _show_stock_removal_result(self, result: StockRemovalResult) -> None:
        old = getattr(self, "_stock_removal_dialog", None)
        if old is not None:
            old.close()
        dialog = QDialog(self)
        dialog.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose, True)
        dialog.setWindowTitle("CarveFoundry — Sampled Stock Removal")
    ...
    def _run_stock_removal(self, *, spacing_mm: float = 0.75) -> bool:
        ...
        def completed(result):
            self._show_stock_removal_result(result)
            self.statusBar().showMessage(
    ...
    def _simulate_stock_removal(self) -> None:
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self._run_stock_removal(spacing_mm=spacing.value())
